...
ConfigurationManager::~ConfigurationManager() {ca->deregister_service(*s);if (consul_factory) {delete consul_factory;}}
...
bool ConfigurationManager::configure ()
{
  ...
  if (!cli)
  {
    ...
    else if ( cli->opt_exist("-consul") && cli->opt_exist("-ip") && cli->opt_exist("-port") )
    {
      ret_val = configure_from_consul( cli->get_opt("-consul"), cli->get_opt("-ip"), cli->get_opt("-port") );
      if (!ret_val) {
        config_logging->error("Configuration from Consul failed");
      }
    }
